...
class AvLabels:
    ...
    @staticmethod
    def is_dga(token, eng_dict_file = 'data/top10000en.txt', n = 3, threshold = 2):
        '''Return True if a given label is (probably) generated by DGA'''
        label = token[0]
        ...
        def get_meaningful_characters_ratio(label):
            max_len = 0
            if len(label) < 3:
                return 0
            for i in range(n, len(label)):
                tuples = ngrams(label, i)
                tmp_sum = 0
                tmp_s = []

                for s in [''.join(t) for t in tuples]:
                ...
                return max_len / int(len(label))

        def get_ngram_normality_score(label):
            tmp_sum = 0                        
            tuples = ngrams(label, n)            
            for s in [''.join(t) for t in tuples]:
                for words in eng_dict:
                    if s in words:
                        tmp_sum += 1

            return tmp_sum / (len(label) - n + 1)

        global non_dga_score_dict
        if label in non_dga_score_dict:
            non_dga_score = non_dga_score_dict[label]
        else:
            meaningful_characters_ratio = get_meaningful_characters_ratio(label)
            ngram_normality_score = get_ngram_normality_score(label)
            non_dga_score = statistics.harmonic_mean([meaningful_characters_ratio, ngram_normality_score])
            non_dga_score_dict[label] = non_dga_score    
    
        if non_dga_score < threshold:
            return True
        else:
            return False
